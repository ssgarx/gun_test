...
function TestMessage({g,u,k,setSelected}) {
    ...
    const handleSubmit = async (team_index)=>{
        ...
        u.once((a)=>{
            const msg = u.get("teams").set({...static_teams[team_index],points:0,user_name:a?.alias})
            if(team_index == 0){
            ...
            setSelected(true)
            g.get('application').get('leaderboard').set({user:a?.alias,team:static_teams[team_index].id,points:0,validations:0})
        })
    }
